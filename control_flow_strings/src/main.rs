...
// fn main() {
//     let  name = "Hello this is a literal string";
//     println!("{:?}", name.bytes().nth(0));
//     for i in 0..=10{
//         print!("{}", i);
//     }
//     println!();
//     let number: i32 = 12;
//     let is_number_prime = control_flow_strings::is_prime(number); 
//     println!("{} is {}", number, if is_number_prime { "prime"} else {"not prime"})
// }
...
fn count(arg: String) {
    ...
    let mut count = 0;
    'mainloop:loop{
        println!("{} ", arg.len()); // Execute this line 8 times, and then break. `print!` doesn't add a newline.
        if count == 8{
            break 'mainloop;
        }
        count += 1;
